...
describe('unwrapName', () => {
  ...
  it('should return a .arb unwrap name transaction and succeed', async () => {
    const tx = await ensInstance.unwrapName('wrapped.arb', {
  ...
  it('should return a regular unwrap name transaction and succeed', async () => {
    const tx = await ensInstance.unwrapName('test.wrapped-with-subnames.arb', {
      newController: accounts[1],
      addressOrIndex: 2,
    })
    expect(tx).toBeTruthy()
