...
describe('transferName', () => {
  ...
  it('should allow a transfer on the registrar', async () => {
    const tx = await ensInstance.transferName('test123.arb', {
  ...
  it('should allow a transfer on the namewrapper', async () => {
    const tx = await ensInstance.transferName('wrapped.arb', {
      newOwner: accounts[2],
    ...
    const result = await nameWrapper.ownerOf(namehash('wrapped.arb'))
    expect(result).toBe(accounts[2])
  })
  it('should allow a transfer on the registry', async () => {
    const tx = await ensInstance.transferName('test.with-subnames.arb', {
      newOwner: accounts[1],
      contract: 'registry',
