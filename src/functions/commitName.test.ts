...
describe('commitName', () => {
  ...
  it('should return a populated commit transaction with extra data and succeed', async () => {
    const { customData, ...popTx } =
      await ensInstance.commitName.populateTransaction('commitment.arb', {
        duration: 31536000,
        owner: accounts[1],
    ...
    const controller = await ensInstance.contracts!.getArbRegistrarController()!
  ...
  it('should return a customised commmit transaction and succeed', async () => {
    ...
    const tx = await ensInstance.commitName('commitment.arb', {
      duration: 31536000,
      owner: accounts[1],
      addressOrIndex: accounts[1],
      secret,
